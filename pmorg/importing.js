...
function importRMG(file) {
  ...
  var steps = lines.slice(2).map(function(line) {
    ...
    if (step == "NRM") {
      ...
      intensity = Number(lineSplit[6]) * 1000;
      dSpec = Number(lineSplit[8]);
      iSpec = Number(lineSplit[9]);
    }
...
function importDIR(file) {
  ...
  var interpretations = lines.map(function(line) {
    ...
    var comment;
    for (let i = 9; i < lineSplit.length; i++) comment += lineSplit[i];
    ...
    return {
      ...
      stepRange: stepRange,
      n: stepN,
      Dspec: undefined, // we have not info about specimen
      Ispec: undefined,
      Dgeo: Dgeo,
      Igeo: Igeo,
      Dstrat: Dstrat,
...
function importPMCSV(file) {
  ...
  if (header[0] == 'ID') {
    ...
    var interpretations = lines.slice(1).map(function(line) {
      ...
      var Dgeo = Number(lineSplit[6]);
      var Igeo = Number(lineSplit[7]);
      var Dstrat = Number(lineSplit[8]);
      var Istrat = Number(lineSplit[9]);
      var mad = Number(lineSplit[10]);

      var comment;
      for (let i = 11; i < lineSplit.length; i++) comment += lineSplit[i];
