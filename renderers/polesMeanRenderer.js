...
function updateMeanTable() {
  ...
  sitesSets.forEach(function(sitesSet, i) {
    var currSpecRows = sitesSet.means.map(function(mean, j) {
      ...
      // }
      // var k = (mean.k) ? mean.k.toFixed(1) : "<span class='text-primary' title='The k for Bingham distribution is ambiguous'>" + '?' + "</span>";
...
function meanTableClickHandler(event) {
  ...
  var sitesSetName = event.target.parentElement.querySelectorAll("td")[1].innerHTML;
  ...
  for (let i = 0; i < tableRows.length; i++) {
    if (tableRows[i].cells[1].innerHTML == sitesSetName) {
  ...
  var selectedSitesSet;
  for (let i = 0; i < sitesSets.length; i++) {
    if (sitesSets[i].name == sitesSetName) {
      selectedSitesSet = sitesSets[i];
      break;
    }
  }
  ...
  //redrawCharts();
  saveLocalStorage(sitesSets);

}

function saveLocalStorage(sitesSets, selectedSitesSet) {
...
function downloadMeansCSV() {
  ...
  const FILENAME = "poleStatistics";

  const CSV_UPHEADER = new Array(
    "from:", "poles", "PMTools beta v" + getVersion(),
  ).join(ITEM_DELIMITER);
  ...
  sitesSets.forEach(function(sitesSet) {
    sitesSet.means.forEach(function(mean) {
  ...
  saveFile("Save pole statistics data", FILENAME, outputMeans);
...
function downloadMeansXLSX() {
  ...
  saveFile("Save pole statistics data", FILENAME, outputMeans, 'xlsx');
